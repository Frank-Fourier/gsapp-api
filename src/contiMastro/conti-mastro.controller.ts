...
import { ContoMastroEntity, ContoMastroWithDetailsEntity } from './entities/conto-mastro.entity';
...
@Get(':id/details')
@ApiOkResponse({ type: ContoMastroWithDetailsEntity })
async findOneWithDetails(@Param('id', ParseIntPipe) id: number) {
  const contoMastro = await this.contiMastroService.findOneWithDetails(id);
  if (!contoMastro) {
    throw new NotFoundException(`ContoMastro with ID ${id} does not exist.`);
  }
  return new ContoMastroWithDetailsEntity(contoMastro);
}
