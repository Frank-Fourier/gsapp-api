...
import { ProfiloAmministratoreEntity, ProfiloAmministratoreWithRelationsEntity } from './entities/amministratore.entity';
...
export class ProfiliAmministratoreController {
  ...
  @Get(':id/dettagli')
  @ApiOkResponse({ type: ProfiloAmministratoreWithRelationsEntity })
  async findOneWithRelations(@Param('id', ParseIntPipe) id: number) {
    const profiloAmministratore = await this.profiliAmministratoreService.findOneWithRelations(id);
    if (!profiloAmministratore) {
      throw new NotFoundException(`ProfiloAmministratore with ID ${id} does not exist.`);
    }
    return new ProfiloAmministratoreWithRelationsEntity(profiloAmministratore);
  }
