...
import { TabellaEntity, TabellaWithRelationsEntity } from './entities/tabella.entity';
...
export class TabelleController {
  ...
  @Get(':id/dettagli')
  @ApiOkResponse({ type: TabellaWithRelationsEntity })
  async findOneWithRelations(@Param('id', ParseIntPipe) id: number) {
    const tabella = await this.tabelleService.findOneWithRelations(id);
    if (!tabella) {
      throw new NotFoundException(`Tabella with ID ${id} does not exist.`);
    }
    return new TabellaWithRelationsEntity(tabella);
  }
  ...
  async update(
    ...
    @Body() updateTabellaDto: UpdateTabellaDto,
  ) {
