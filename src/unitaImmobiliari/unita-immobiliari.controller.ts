...
import { UnitaImmobiliareEntity, UnitaImmobiliareWithTransazioniEntity } from './entities/unita-immobiliare.entity';
...
export class UnitaImmobiliariController {
  ...
  @Get(':id/transazioni')
  @ApiOkResponse({ type: UnitaImmobiliareWithTransazioniEntity })
  async findOneWithTransazioni(@Param('id', ParseIntPipe) id: number) {
    const risorsa = await this.unitaImmobiliariService.findOneWithTransazioni(id);
    if (!risorsa) {
      throw new NotFoundException(`Risorsa with ID ${id} does not exist.`);
    }
    return new UnitaImmobiliareWithTransazioniEntity(risorsa);
  }
